...
export const PlatformAdminPage: React.FC = () => {
  ...
  const [pendingStudios, setPendingStudios] = useState<PlatformStudioResponse[]>([]);
  const [allStudios, setAllStudios] = useState<PlatformStudioResponse[]>([]);
  ...
  const loadData = async () => {
    ...
    try {
      ...
      setPendingStudios(pendingRes);
      setAllStudios(allRes);
    } catch (err: any) {
      setError(err.message || 'Failed to fetch platform admin data. Please check connection and authentication status.');
    } finally {
      setLoading(false);
    }
  };
