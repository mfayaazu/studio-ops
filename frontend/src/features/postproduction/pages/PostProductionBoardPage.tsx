    ...
    <main className="space-y-8 pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-gradient-to-r from-slate-900/60 to-indigo-950/20 border border-slate-800/60 rounded-2xl p-6 shadow-xl relative overflow-hidden">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-1.5 px-3 py-1.5 bg-violet-500/10 border border-violet-500/20 rounded-full text-[11px] font-bold text-violet-400">
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
        {/* Total / Active */}
        <div className="bg-[#0b1222]/40 border border-slate-800/60 rounded-2xl p-4.5 space-y-2.5 shadow-md">
          ...
          <div>
            <h3 className="text-2xl font-black text-white font-mono">{activeTasks}</h3>
            <p className="text-[10px] text-slate-500 mt-1 font-semibold">Total: {totalTasks} tasks in backlog</p>
          </div>
        </div>
        ...
        <div className="bg-[#0b1222]/40 border border-slate-800/60 rounded-2xl p-4.5 space-y-2.5 shadow-md">
          ...
          <div>
            <h3 className="text-2xl font-black text-white font-mono">{inProgressTasks}</h3>
            <p className="text-[10px] text-slate-500 mt-1 font-semibold">Actively culling & grading</p>
          </div>
        </div>
        ...
        <div className="bg-[#0b1222]/40 border border-slate-800/60 rounded-2xl p-4.5 space-y-2.5 shadow-md">
          <div className="flex justify-between items-center text-slate-500">
            <span className="text-[11px] font-bold uppercase tracking-wider">Blocked</span>
            <AlertOctagon className="h-4.5 w-4.5 text-rose-400" />
          </div>
          <div>
            <h3 className="text-2xl font-black text-white font-mono">{blockedTasks}</h3>
            <p className="text-[10px] text-slate-500 mt-1 font-semibold">Requires PM intervention</p>
          </div>
        </div>
        ...
        <div className="bg-[#0b1222]/40 border border-slate-800/60 rounded-2xl p-4.5 space-y-2.5 shadow-md">
          ...
          <div>
            <h3 className="text-2xl font-black text-white font-mono">{completedTasks}</h3>
            <p className="text-[10px] text-slate-500 mt-1 font-semibold">Ready for client delivery</p>
          </div>
        </div>
        ...
        <div className="bg-[#0b1222]/40 border border-slate-800/60 rounded-2xl p-4.5 space-y-2.5 shadow-md">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-500 mt-1 font-semibold">Actual vs Estimated effort</p>
          </div>
        </div>
      </div>
      ...
      {apiStatus !== 'offline' && (
        <div className="bg-[#0b1222]/40 border border-slate-800/60 rounded-2xl p-4.5 shadow-lg space-y-4">
          {/* Top Row: Search & Sort & Clear */}
          <div className="flex flex-col md:flex-row gap-4 items-stretch md:items-center justify-between pb-3 border-b border-slate-800/40">
            {/* Search Input & Active Count */}
            <div className="flex items-center gap-3 flex-grow max-w-xl">
              <div className="relative flex-grow">
                <Search className="absolute left-3 top-2.5 h-4 w-4 text-slate-500 animate-pulse" />
                <input
                  type="text"
                  placeholder="Search tasks by title or description..."
                  value={filterSearch}
                  onChange={(e) => setFilterSearch(e.target.value)}
                  className="w-full pl-9 pr-9 py-2 bg-slate-950/60 border border-slate-800 hover:border-slate-700 text-slate-200 placeholder-slate-500 rounded-lg text-xs transition-colors outline-none focus:ring-1 focus:ring-violet-500/50"
                />
                {filterSearch && (
                  <button
              ...
              {activeFilterCount > 0 && (
                <span className="shrink-0 px-2.5 py-1 bg-violet-500/10 border border-violet-500/20 text-violet-400 rounded-full text-[10px] font-bold">
            ...
            <div className="flex items-center gap-3 shrink-0">
              <div className="flex items-center gap-2">
                <span className="text-[10px] uppercase tracking-wider font-bold text-slate-500">Sort By</span>
                <select
                  value={sortBy}
                  onChange={(e) => setSortBy(e.target.value)}
                  className="bg-slate-950/60 border border-slate-800 hover:border-slate-700 text-slate-350 hover:text-white rounded-lg text-xs py-1.5 px-3 transition-colors cursor-pointer outline-none focus:ring-1 focus:ring-violet-500/50"
                >
              ...
              {activeFilterCount > 0 && (
                <button
                  ...
                  className="py-1.5 px-3 bg-slate-800 hover:bg-slate-700 hover:text-white text-slate-300 rounded-lg text-xs font-semibold transition-all border border-slate-750 flex items-center gap-1 cursor-pointer"
                >
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3">
            ...
            <div className="flex flex-col gap-1">
              <label className="text-[10px] uppercase tracking-wider font-bold text-slate-500">Assignee</label>
              <select
                value={filterAssignee}
                onChange={(e) => setFilterAssignee(e.target.value)}
                className="bg-slate-950/60 border border-slate-800 hover:border-slate-700 text-slate-350 hover:text-white rounded-lg text-xs py-1.5 px-2 transition-colors cursor-pointer outline-none focus:ring-1 focus:ring-violet-500/50"
              >
            ...
            {/* Priority Filter */}
            <div className="flex flex-col gap-1">
              <label className="text-[10px] uppercase tracking-wider font-bold text-slate-500">Priority</label>
              <select
            ...
            {/* Task Type Filter */}
            <div className="flex flex-col gap-1">
              <label className="text-[10px] uppercase tracking-wider font-bold text-slate-500">Task Type</label>
              <select
            ...
            {/* Due Date Filter */}
            <div className="flex flex-col gap-1">
              <label className="text-[10px] uppercase tracking-wider font-bold text-slate-500">Due Date</label>
              <select
                value={filterDueDate}
            ...
            {/* Project Filter */}
            <div className="flex flex-col gap-1">
              <label className="text-[10px] uppercase tracking-wider font-bold text-slate-500">Project</label>
              <select
                value={filterProject}
            ...
            {/* Deliverable Filter */}
            <div className="flex flex-col gap-1">
              <label className="text-[10px] uppercase tracking-wider font-bold text-slate-500">Deliverable</label>
              <select
