...
import { Sparkles, MessageSquare, Compass, LayoutGrid, CheckSquare, Loader2, Plus, X, AlertTriangle, TrendingUp, ChevronDown, ChevronUp, Search } from 'lucide-react';
...
export const FollowUpCenterPage: React.FC = () => {
  ...
  const [showInsights, setShowInsights] = useState<boolean>(() => {
    return localStorage.getItem('studioops.followup.insights.visible') === 'true';
  });

  const [showFilters, setShowFilters] = useState<boolean>(() => {
    return localStorage.getItem('studioops.followup.filters.visible') === 'true';
  });
  ...
  const isAnyOffline = leadApiStatus === 'offline' || tasksApiStatus === 'offline' || templatesApiStatus === 'offline' || sequencesApiStatus === 'offline';
  const isAllConnected = leadApiStatus === 'connected' && tasksApiStatus === 'connected' && templatesApiStatus === 'connected' && sequencesApiStatus === 'connected';
  ...
  return (
    <main className="space-y-4 pb-12">
      ...
      <div className="flex items-center justify-between border-b border-slate-800/60 pb-2">
        ...
        <div className="flex items-center gap-1.5 px-2 py-0.5 bg-emerald-500/10 border border-emerald-500/20 rounded-full text-[9px] font-bold text-emerald-400">
      ...
      <div className="flex flex-row items-center justify-between border-b border-slate-800/80 pb-0 gap-2 w-full flex-nowrap overflow-visible">
        <div className="flex items-center gap-0.5 overflow-x-auto scrollbar-none flex-shrink">
          <button
            onClick={() => setActiveTab('pipeline')}
            className={`flex items-center gap-1 px-2.5 py-1.5 text-[10px] font-bold uppercase tracking-wider transition-all duration-250 border-b-2 whitespace-nowrap ${
              activeTab === 'pipeline'
        ...
        <div className="flex items-center gap-1.5 flex-shrink-0 py-1 text-[11px] text-xs">
          {/* Collapsed Dropdown for mobile */}
          <div 
          ...
          >
            ...
            >
              <span className={`h-1.5 w-1.5 rounded-full ${isAnyOffline ? 'bg-rose-500' : isAllConnected ? 'bg-emerald-500 animate-pulse' : 'bg-amber-500'}`} />
              <span>Systems: {isAnyOffline ? 'Attention' : 'Connected'}</span>
              <ChevronDown className="h-2.5 w-2.5 ml-0.5" />
            </button>
            
            {showStatusDropdown && (
              <div className="absolute right-0 mt-1 w-40 bg-slate-900 border border-slate-800 rounded-lg shadow-xl p-2.5 z-50 space-y-1.5 animate-fadeIn">
                <div className="text-[9px] uppercase tracking-wider font-mono text-slate-500 font-bold border-b border-slate-800 pb-1 mb-1 px-1">
                  System Details
                </div>
                <div className="flex items-center justify-between px-1 text-[10px]">
                  <span className="text-slate-400">Leads</span>
                  <span className={leadApiStatus === 'connected' ? 'text-emerald-400 font-bold' : leadApiStatus === 'offline' ? 'text-rose-400 font-bold' : 'text-sky-400'}>
                    {leadApiStatus}
                  </span>
                </div>
                <div className="flex items-center justify-between px-1 text-[10px]">
                  <span className="text-slate-400">Follow-ups</span>
                  <span className={tasksApiStatus === 'connected' ? 'text-emerald-400 font-bold' : tasksApiStatus === 'offline' ? 'text-rose-400 font-bold' : 'text-sky-400'}>
                    {tasksApiStatus}
                  </span>
                </div>
                <div className="flex items-center justify-between px-1 text-[10px]">
                  <span className="text-slate-400">Templates</span>
                  <span className={templatesApiStatus === 'connected' ? 'text-emerald-400 font-bold' : templatesApiStatus === 'offline' ? 'text-rose-400 font-bold' : 'text-sky-400'}>
                    {templatesApiStatus}
                  </span>
                </div>
                <div className="flex items-center justify-between px-1 text-[10px]">
                  <span className="text-slate-400">Sequence</span>
                  <span className={sequencesApiStatus === 'connected' ? 'text-emerald-400 font-bold' : sequencesApiStatus === 'offline' ? 'text-rose-400 font-bold' : 'text-sky-400'}>
                    {sequencesApiStatus}
                  </span>
                </div>
              </div>
          ...
          {/* Desktop view: Individual status pills */}
          <div className="hidden md:flex items-center gap-1 px-2 py-0.5">
            {leadApiStatus === 'connected' && (
              <span className="px-2 py-0.5 bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 rounded-full text-[10px] font-bold flex items-center gap-1 font-mono animate-fadeIn">
                <span className="h-1 w-1 rounded-full bg-emerald-500 animate-pulse" />
                Leads: Connected
              </span>
            )}
            {leadApiStatus === 'empty' && (
              <span className="px-2 py-0.5 bg-sky-500/10 border border-sky-500/20 text-sky-400 rounded-full text-[10px] font-bold flex items-center gap-1 font-mono">
                <span className="h-1 w-1 rounded-full bg-sky-500" />
                Leads: Empty
              </span>
            )}
            {leadApiStatus === 'offline' && (
              <span className="px-2 py-0.5 bg-rose-500/10 border border-rose-500/20 text-rose-455 rounded-full text-[10px] font-bold flex items-center gap-1 font-mono">
            ...
            {tasksApiStatus === 'connected' && (
              <span className="px-2 py-0.5 bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 rounded-full text-[10px] font-bold flex items-center gap-1 font-mono">
                <span className="h-1 w-1 rounded-full bg-emerald-500 animate-pulse" />
            ...
            {tasksApiStatus === 'offline' && (
              <span className="px-2 py-0.5 bg-rose-500/10 border border-rose-500/20 text-rose-455 rounded-full text-[10px] font-bold flex items-center gap-1 font-mono animate-pulse">
                <span className="h-1 w-1 rounded-full bg-rose-500" />
                Follow-ups: Offline
              </span>
            )}
      ...
      {activeTab === 'pipeline' && (
        <div className="space-y-2">
          <div className="flex flex-wrap items-center justify-between gap-2 bg-[#090f1e]/60 border border-slate-800/50 rounded-xl p-1.5 shadow-md">
            <div className="flex items-center gap-2">
              <button
                onClick={() => {
                  setShowFilters(prev => {
                    const next = !prev;
                    localStorage.setItem('studioops.followup.filters.visible', String(next));
                    return next;
                  });
                }}
              ...
              <button
                onClick={() => {
                  setShowInsights(prev => {
                    const next = !prev;
                    localStorage.setItem('studioops.followup.insights.visible', String(next));
                    return next;
                  });
                }}
                className={`px-2.5 py-1.5 border rounded-lg text-xs font-semibold transition-all flex items-center gap-1 cursor-pointer ${
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="px-2.5 py-1.5 bg-slate-900 border border-slate-800 hover:bg-slate-800 text-slate-350 hover:text-white rounded-lg text-xs font-semibold flex items-center gap-1.5 transition-colors cursor-pointer"
              >
              ...
              <button
                ...
                className="py-1.5 px-3 bg-violet-600 hover:bg-violet-500 text-white rounded-lg text-xs font-semibold transition-all shadow-md flex items-center gap-1 cursor-pointer"
              >
              ...
              <div className="flex flex-col md:flex-row gap-3 items-center justify-between">
                ...
                <div className="relative w-full md:max-w-md">
                  <input
                    ...
                    className="w-full bg-[#070b14]/80 border border-slate-800 text-slate-200 rounded-lg pl-3 pr-8 py-1.5 text-xs focus:ring-1 focus:ring-violet-500 focus:outline-none"
                  />
                  {searchQuery && (
                    <button
                ...
                viewMode === 'board' ? (
                  ...
                  <div className="bg-[#0b1222]/50 border border-slate-800/60 rounded-xl p-2 shadow-xl overflow-x-auto">
                    <table className="w-full text-left border-collapse text-[11px] text-slate-350">
                      <thead>
                        <tr className="border-b border-slate-850 text-[9px] uppercase font-mono tracking-wider text-slate-500">
                          <th className="py-2 px-3 font-bold">Client Name</th>
                          <th className="py-2 px-3 font-bold">Primary Event</th>
                          ...
                          <th className="py-2 px-3 font-bold">Next Follow-up</th>
                          <th className="py-2 px-3 font-bold">Total / Remaining</th>
                      ...
                      <tbody>
                        {filteredLeads.map(lead => (
                          ...
                          >
                            <td className="py-1.5 px-3 font-bold text-slate-200">{lead.clientName}</td>
                            <td className="py-1.5 px-3 font-medium text-slate-400">{lead.projectTitle}</td>
                            <td className="py-1.5 px-3">
                              <span className="px-1.5 py-0.2 rounded text-[8px] font-semibold bg-slate-800 text-slate-300 border border-slate-700/50">
                                {lead.stage}
                              </span>
                            </td>
                            <td className="py-1.5 px-3">
                              <span className="px-1.5 py-0.2 rounded text-[8px] font-bold bg-slate-800 text-slate-300 border border-slate-700/50">
                                {lead.priority}
                              </span>
                            </td>
                            <td className="py-1.5 px-3 font-semibold">
                              <span className="px-1.5 py-0.2 rounded text-[8px] bg-slate-800 text-slate-300 border border-slate-700/50">
                                {lead.paymentStatus}
                              </span>
                            </td>
                            <td className="py-1.5 px-3 font-mono text-[10px]">{lead.nextFollowUp}</td>
                            <td className="py-1.5 px-3 font-mono text-[10px] text-slate-300">
                              <div>{formatCurrencyINR(lead.quotationTotal)}</div>
                              <div className="text-[9px] text-slate-500">Rem: {formatCurrencyINR(lead.amountRemaining)}</div>
                            </td>
                            <td className="py-1.5 px-3">{lead.city || 'TBD'}</td>
                          </tr>
                        ))}
      ...
      <LeadDetailDrawer
        lead={selectedLead}
        task={
          selectedLead && Array.isArray(dueTasks)
            ? (dueTasks.find(t => {
                if ((t as any).leadId && (t as any).leadId === selectedLead.id) return true;
                if (selectedLead.clientId && t.clientId === selectedLead.clientId) return true;
                if (selectedLead.projectId && t.projectId === selectedLead.projectId) return true;
                return false;
              }) || null)
            : null
        }
