...
public class CommunicationLogService {
    ...
    public List<CommunicationLogResponse> listLogs() {
        return listLogsForStudio(TenantConstants.DEFAULT_STUDIO_ID);
    }
    ...
    public CommunicationLogResponse getLogById(UUID id) {
        CommunicationLog log = communicationLogRepository.findByIdAndStudioId(id, TenantConstants.DEFAULT_STUDIO_ID)
    ...
    public List<CommunicationLogResponse> listLogsByProjectId(UUID projectId) {
        projectRepository.findByIdAndStudioId(projectId, TenantConstants.DEFAULT_STUDIO_ID)
            .orElseThrow(() -> new ResourceNotFoundException("Project not found with id: " + projectId));
        return communicationLogRepository.findByProjectIdAndStudioId(projectId, TenantConstants.DEFAULT_STUDIO_ID).stream()
                .map(CommunicationLogMapper::toResponse)
                .toList();
    }
    ...
    public List<CommunicationLogResponse> listLogsByClientId(UUID clientId) {
        clientRepository.findByIdAndStudioId(clientId, TenantConstants.DEFAULT_STUDIO_ID)
            .orElseThrow(() -> new ResourceNotFoundException("Client not found with id: " + clientId));
        return communicationLogRepository.findByClientIdAndStudioId(clientId, TenantConstants.DEFAULT_STUDIO_ID).stream()
                .map(CommunicationLogMapper::toResponse)
                .toList();
    }
    ...
    public List<CommunicationLogResponse> listLogsByTaskId(UUID taskId) {
        followUpTaskRepository.findByIdAndStudioId(taskId, TenantConstants.DEFAULT_STUDIO_ID)
            .orElseThrow(() -> new ResourceNotFoundException("Follow-up task not found with id: " + taskId));
        return communicationLogRepository.findByFollowUpTaskIdAndStudioId(taskId, TenantConstants.DEFAULT_STUDIO_ID).stream()
                .map(CommunicationLogMapper::toResponse)
                .toList();
    }
}
